...
//import 'flutter_slider_drawer.dart';
import 'drawer.dart';
...
class MainPageBuild extends State<MainPage> with TickerProviderStateMixin {
  ...
  final List<Widget> pages = [
    const PageNoDie(child: Profile()),
    const PageNoDie(child: HomePage()),
    const PageNoDie(child: Groups())
  ];
  ...
  navigatingScreens(int index) {
    setState(() {
      ...
      if (index == 0) {
        currentName = 'P R O F I L E';
      }
      if (index == 1) {
        currentName = 'H O M E';
      }
      if (index == 2) {
        currentName = 'G R O U P S';
      }
    });
  }
            ...
            body: Stack(
              ...
              children: [
                Container(
                  color: Colors.black,
                  child: Animations()
                      .buildBackground(rotAnim!, radAnim!, traY!, opac!),
                ),
                PageView(
                  controller: pageCont,
